...
If incomming connection -> start socket to the incomming-ip:10000 (django server) and start sending data to that ip on port 10000.
Tested on both windows 8.1 and manjaro

See git repository for more information: 
'''
...
def main(incoming_socket):
        ...
        with conn:
            ...
            conn.setblocking(1)
            session_id = str(conn.recv(16).decode())
            session_id_x = 's1'+ str(session_id)
            session_id_y = 's2'+ str(int(session_id) + 1)
            session_id_z = 's3'+ str(int(session_id) + 2)
            print('Session id set to (1+x,2+y,3+z): ('+  session_id_x +','+session_id_y+','+session_id_z+')')  
            ...
            #Send session id back to django server with 300ms delay to prevent buggs
            outgoing_socket.send(session_id_x.encode())
            time.sleep(0.3)
            outgoing_socket.send(session_id_y.encode())
            time.sleep(0.3)
            outgoing_socket.send(session_id_z.encode())
            ...
            while True:
                try:
                    ...
                    send_data_x = 'x'+str(111)
                    send_data_y = 'y'+str(222)
                    send_data_z = 'z'+str(333)
                    outgoing_socket.send(send_data_x.encode())
                    time.sleep(0.2)
                    outgoing_socket.send(send_data_y.encode())
                    time.sleep(0.2)
                    outgoing_socket.send(send_data_z.encode())
                    time.sleep(0.2)
                    ...
                    continue    #Is raised if no data can be received from incomming_socket (inData = conn.recv(16))
